...
[![版本](https://img.shields.io/badge/version-1.3.0-blue.svg)](https://github.com/hulion/nextpua)
...
```bash
claude install-plugin github:hulion/nextpua
```
...
| 第 4 次 | L3 績效考核 | 完成 7 項檢查清單 + 3 個全新假設 |
...
7 項檢查清單全部完成仍未解決時，輸出結構化失敗報告，而非無限迴圈。
...
.claude-plugin/
├── plugin.json          # 插件元資料
└── marketplace.json     # 市集元資料
commands/
└── pua.md               # 啟動指令 (/c2l:pua)
skills/
└── pua/
    ├── SKILL.md          # 核心邏輯（鐵律、升級、方法論、風格）
    └── references/
